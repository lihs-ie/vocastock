        ...
        match stream {
            Ok(stream) => {
                if let Err(error) = handle_connection(stream, readiness_path.as_str()) {
                    eprintln!("{} request handling error: {}", command_api::SERVICE_NAME, error);
                }
            }
            Err(error) => {
                eprintln!("{} accept error: {}", command_api::SERVICE_NAME, error);
            }
        }
    }
}
...
fn handle_connection(mut stream: TcpStream, readiness_path: &str) -> std::io::Result<()> {
    ...
    match path {
        path if path == readiness_path => respond(
            &mut stream,
            "200 OK",
            format!("{} ready", command_api::SERVICE_NAME).as_str(),
        ),
        FIREBASE_DEPENDENCIES_PATH => firebase_dependency_response(&mut stream),
        "/" => respond(
            &mut stream,
            "200 OK",
            command_api::visible_guarantee(),
        ),
