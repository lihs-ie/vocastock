//! Deterministic in-memory CommandStore used by command-api unit
//! tests. Lives under `tests/support/` so the production binary never
//! links this double — the real adapter is `FirestoreCommandStore`.

use std::collections::BTreeMap;
use std::sync::Mutex;

use command_api::{
    vocabulary_expression_for, AcceptedCommandResult, CommandStore, DuplicateReuseResult,
    PlannedNewRegistration, PlannedReuseRegistration, RegisterVocabularyExpressionCommand,
    StoreDecision, StoredRegistration, EXPLANATION_STATE_FAILED_FINAL,
