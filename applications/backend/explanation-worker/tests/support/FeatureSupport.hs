...
constExplanationWorkerService :: String
constExplanationWorkerService = "explanation-worker"
...
startStableWorker runtime = do
  ...
  runCommandStrict (runtimeRepoRoot runtime) "docker" (args ++ [constExplanationWorkerService])
    ...
    poll remaining = do
      ...
      if "entered stable-run mode" `isInfixOf` logs
        then pure ()
          ...
          if remaining <= 0
            then error "explanation-worker did not enter stable-run mode in time"
            else threadDelay 1000000 >> poll (remaining - 1)

workerLogs :: FeatureRuntime -> IO String
workerLogs runtime = do
  args <- composeArgs runtime ["logs", "--no-color", constExplanationWorkerService]
  ...
  args <-
    composeArgs
      ...
      [ "run",
        ...
        "-e",
        "VOCAS_EXPLANATION_WORKFLOW_SCENARIO=" ++ scenario,
        constExplanationWorkerService
      ]
...
writeFeatureEnvFile repoRoot baseEnvFile firestorePort storagePort authPort = do
  ...
  let envFile = logsDir </> ("explanation-worker-feature-" ++ suffix ++ ".env")
      contents =
...
buildWorkerImage repoRoot composeFile envFile = do
  ...
  if skipBuild
    then pure ()
      ...
      args <- composeArgs' composeFile envFile ["build", constExplanationWorkerService]
...
removeStaleWorkerContainer repoRoot composeFile envFile = do
  args <- composeArgs' composeFile envFile ["rm", "-sf", constExplanationWorkerService]
...
parseValidationResult stdout =
  case filter ("VOCAS_EXPLANATION_RESULT " `isPrefixOf`) (lines stdout) of
