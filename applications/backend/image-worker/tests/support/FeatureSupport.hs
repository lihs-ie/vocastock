...
constImageWorkerService :: String
constImageWorkerService = "image-worker"
...
startStableWorker runtime = do
  ...
  runCommandStrict (runtimeRepoRoot runtime) "docker" (args ++ [constImageWorkerService])
    ...
    poll remaining = do
      ...
      if "entered stable-run mode" `isInfixOf` logs
        then pure ()
          ...
          if remaining <= 0
            then error "image-worker did not enter stable-run mode in time"
            else threadDelay 1000000 >> poll (remaining - 1)

workerLogs :: FeatureRuntime -> IO String
workerLogs runtime = do
  args <- composeArgs runtime ["logs", "--no-color", constImageWorkerService]
  ...
  args <-
    composeArgs
      ...
      [ "run",
        ...
        "-e",
        "VOCAS_IMAGE_WORKFLOW_SCENARIO=" ++ scenario,
        constImageWorkerService
      ]
...
writeFeatureEnvFile repoRoot baseEnvFile firestorePort storagePort authPort = do
  ...
  let envFile = logsDir </> ("image-worker-feature-" ++ suffix ++ ".env")
      contents =
...
buildWorkerImage repoRoot composeFile envFile = do
  ...
  if skipBuild
    then pure ()
      ...
      args <- composeArgs' composeFile envFile ["build", constImageWorkerService]
...
removeStaleWorkerContainer repoRoot composeFile envFile = do
  args <- composeArgs' composeFile envFile ["rm", "-sf", constImageWorkerService]
...
parseValidationResult stdout =
  case filter ("VOCAS_IMAGE_RESULT " `isPrefixOf`) (lines stdout) of
