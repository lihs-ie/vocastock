...
## Decision: Apple 系ツールチェーンは Xcode 26.3 と macOS 15.6+ を採用する

**Rationale**: Apple の Xcode 要件では Xcode 26.3 は macOS 15.6 以降を要求する。
GitHub Actions の `macos-15` image は 2026-04 時点で macOS 15.7.4 を提供しており、
ローカル macOS と CI の基準線を揃えやすい。beta の Xcode 26.4 系へ寄せるより、
公開済み stable の 26.3 を採る方が再現性と CI 互換性に優れる。

**Alternatives considered**:

- Xcode 26.4 beta を先行採用する
- 旧系統の Xcode へ下げて CI との差分を増やす

## Decision: Android 系ツールチェーンは Android Studio Panda 2 (2025.3.2) を採用する

**Rationale**: Android Studio の公式 release 一覧で Panda 2 (2025.3.2) は stable 系統として
提示され、AGP 9.1 系までを対象にできる。Canary を避けつつ、古い Otter/Narwhal 系より
新しい安定版を使うことで、Flutter の Android toolchain 互換性と security fix の両立を
図りやすい。
...
## Decision: コンテナ基盤は Docker Desktop 4.60.1 を採用する

**Rationale**: Docker Desktop の公式 release notes では 4.60.1 が最新 stable であり、
過去 6 か月より古い版は配布対象外とされている。さらに 4.57.0 以降には security fix が
含まれているため、現在配布されている最新 stable を基準にするのが最も安全である。
