...
1. [research.md](/Users/lihs/workspace/vocastock/specs/021-explanation-worker-implementation/research.md) を読み、
   Haskell runtime baseline、registration-origin initial slice、二段階 success、malformed payload の
   terminal 扱い、duplicate handling、テスト方針の判断理由を確認する
...
3. `cargo test --manifest-path /Users/lihs/workspace/vocastock/applications/backend/explanation-worker/tests/Cargo.toml --test feature -- --nocapture`
...
9. feature テストは Rust のコードから Docker container と Firebase emulator を使って動くこと
10. runtime validation では `success`、`retryable-failure`、`terminal-failure` を別シナリオとして再現し、`VOCAS_EXPLANATION_RESULT` を summary に記録すること
